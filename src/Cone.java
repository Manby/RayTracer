public class Cone extends Shape {
    ...
    private boolean isInValidHalf(Vector3D point) {
        return (axis.dot(point.copy().subtract(vertex)) >= 0);
    }

    @Override
    public Vector3D getNormalAt(Vector3D point) {
        Vector3D T = point.copy().subtract(vertex);
        double coef = T.dot(T) / axis.dot(T);

        return point.copy().subtract(vertex.copy().add(axis.copy().scale(coef))).normalise();
    }
    ...
    public Hit getFirstHit(Ray ray) throws DoesNotHitException {
        ...
        Vector3D firstPOI = O.copy().add(D.copy().normalise().scale(s2));
        if (s2 > 0 && isInValidHalf(firstPOI)) {
            return new Hit(ray, this, firstPOI, getNormalAt(firstPOI));
        }

        Vector3D secondPOI = O.copy().add(D.copy().normalise().scale(s1));
