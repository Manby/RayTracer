public class Sphere extends Shape {
    ...
    public Vector3D getNormalAt(Vector3D point) {
        return point.copy().subtract(center).normalise();
    }
    ...
    public Hit getFirstHit(Ray ray) throws DoesNotHitException {
        ...
        double a = D.dot(D);
        double b = 2 * D.dot(O.copy().subtract(center));
        double c = (O.copy().subtract(center)).dot(O.copy().subtract(center)) - (radius*radius);
        ...
        Vector3D poi = O.copy().add(D.copy().normalise().scale(sMin));
