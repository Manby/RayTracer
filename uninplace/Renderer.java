...
public class Renderer {
    ...
    private Colour illuminate(Hit hit) {
        ...
        // Ambient component
        colour = colour.add(scene.getAmbientLight().scale(ka).scale(shapeColour));
        ...
        for (Light l : scene.getLights()) {
            ...
            Ray shadowRay = new Ray(hit.getPosition().add(N.scale(1E-10)), L);
            try {
            ...
            dotDiffuse = L.dot(N);
            if (dotDiffuse > 0) {
                colour = colour.add(illumination.scale(kd * dotDiffuse).scale(shapeColour));
            }

            // Specular component
            dotSpecular = R.dot(V);
            if (dotSpecular > 0) {
                colour = colour.add(illumination.scale(ks * Math.pow(dotSpecular, n)));
            }
        }
    ...
    private Colour trace(Ray ray, int numBounces) {     // Traces a ray and calculates its colour
        try {
            ...
            if (numBounces > 0 && reflectivity > 0) {
                colour = colour.add(trace(hit.getReflectionRay(), numBounces-1).scale(reflectivity));   // Create a new reflection ray and recurse
            }
