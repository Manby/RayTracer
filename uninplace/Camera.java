public class Camera {
    ...
    Camera(double x, double y, double z, double nx, double ny, double nz, double d, double width, double height) {
        ...
        yAxis = facing.cross(xAxis).normalise();
    }
    ...
    public Ray getRay(double xFraction, double yFraction) {
        ...
        */
        ...
        Vector3D xStep = xAxis.normalise().scale(width*(xFraction-0.5));
        Vector3D yStep = yAxis.normalise().scale(height*(yFraction-0.5));
        return new Ray(position, facing.normalise().scale(d).add(xStep).add(yStep));
    }
}
